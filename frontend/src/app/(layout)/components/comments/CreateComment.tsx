...
export default function CreateComment({ feed }: userProps) {
  ...
  const userTypeClass =
    styles[userContext.userType == "BLUE" ? "blueLight" : "redLight"] || "";
  const openWriteBox = () => {
  ...
  return (
    <div className={`${styles.CreateCommentContainer} cursorPointer px-4`}>
      <Stack direction="horizontal" gap={3}>
        <div>
          <p className={`${styles.userName} ${userTypeClass}`}>
            {userContext.userName}
          </p>
        </div>
        <div>
          <p
            className={`${styles.commentPlaceHolder} fontGray2`}
            onClick={openWriteBox}
          >
      ...
      {showWriteBox ? (
        <CreateCommentPopUp setShowWriteBox={setShowWriteBox} feed={feed} />
      ) : null}
    </div>
  );
}
