...
export default function Home() {
  ...
  const fetchFeeds = () => {
    ...
    const url = `http://localhost:8090/api/v1/feeds?lastFeedId=${
  ...
  const updateMyFeed = (updatedFeed: Types.Feed) => {
    setFeeds((prevFeeds) =>
      prevFeeds.map((feed) =>
        feed.feedId === updatedFeed.feedId ? updatedFeed : feed
      )
    );
  };
  ...
  const deleteMyFeed = (deletedFeed: Types.Feed) => {
    setFeeds((prevFeeds) =>
      prevFeeds.filter((feed) => feed.feedId !== deletedFeed.feedId)
    );
  };
